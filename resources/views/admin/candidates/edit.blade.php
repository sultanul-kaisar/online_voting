                        ...
                        <div class="col-md-12 col-xl-12">
                            <div class="card sale-card" style="min-height:550px!important;">
                                <div class="card-header">
                                    <h5 class="card-title">Update {{ $candidate->name }}</h5>
                                ...
                                <div class="page-body">
                                    <div class="card-block">
                                        <form method="POST" id="submit-form" action="{{ route('candidate.update', $candidate->id)}}" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                                ...
                                                <div class="col-lg-8">
                                                    <select name="vote_category_id" id="" class="form-control select-search @error('vote_category_id') is-invalid @enderror">
                                                        ...
                                                        @foreach($categories as $category)
                                                            <option value="{{ $category->id }}" {{ (old('vote_category_id', $candidate->vote_category_id) ==  $category->id) ? 'selected=selected':'' }}>{{ $category->title }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-8">
                                                    <select name="status" class="form-control select-search @error('status') is-invalid @enderror">
                                                        <option value="active" {{ (old('status', $candidate->status) == 'active' ) ? 'selected=selected':''}}>Enabled</option>
                                                        <option value="inactive" {{ (old('status', $candidate->status) == 'inactive' ) ? 'selected=selected':''}}>Disabled</option>
                                                    </select>
                                                    @error('status')
                                                    <div class="text-danger">{{ $message }}</div>
                                                    @enderror
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-8">
                                                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" onkeyup="nameSlug(this.value)" placeholder="Full Name" value="{{ old('name', $candidate->name) }}">
                                                    @error('name')
                                                    <div class="text-danger">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-8">
                                                    <input type="text" id="slug" name="slug" class="form-control @error('slug') is-invalid @enderror" placeholder="Slug" value="{{ old('slug', $candidate->slug) }}">
                                                    @error('slug')
                                                    <div class="text-danger">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            @if(!is_null($candidate->image))
                                                <div class="row mb-3">
                                                    <div class="col-8 offset-4">
                                                        <img src="{{ asset('storage/uploads/candidates/'. $candidate->image) }}" class="img-fluid" style="width:20%;">
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-8">
                                                    <textarea id="summernote" name="description" class="form-control" cols="30" rows="10" placeholder="Description">{{ old('description', $candidate->description) }}</textarea>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
